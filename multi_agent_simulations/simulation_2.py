...
def compute_update(pos, vel, w1, w2):
    ...
    attraction_mask = dist < ATTRACTION_RADIUS          # (N,N,) Boolean
    ...
    # Scale by 1 / distance^2 (adding a tiny epsilon to prevent division by zero on perfect overlaps)
    safe_dist_sq = dist**2 + 1e-8
    repulsion_vectors = -unit_vectors * (repulsion_mask[:, :, np.newaxis] / safe_dist_sq[:, :, np.newaxis])
    ...
    if np.any(exceed_max):
        new_vel[exceed_max] = (new_vel[exceed_max] / speeds[exceed_max, np.newaxis]) * V_MAX
