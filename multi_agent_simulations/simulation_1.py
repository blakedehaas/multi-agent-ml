...
# Initiate arena with random positions, angles, and velocities
...
def compute_update(pos, vel, w1, w2):
    N = pos.shape[0] # pos here is (N,2)

    # Calculate all distances between all agents
    diff = pos[np.newaxis, :, :] - pos[:, np.newaxis, :]    # r_ij = (N,N,2)
    dist = np.linalg.norm(diff, axis=2)                     # d_ij = (N,N,)
    np.fill_diagonal(dist, 1.0) # prevents division by 0, we'll zero out this later
    
    unit_vectors = diff / dist[:, :, np.newaxis]            # u_ij      (N,N,2) / (N,N,1)
    for i in range(N):
        unit_vectors[i, i] = [0.0, 0.0] # there is no distance between identity node
        
    # Attraction
    F_attraction = np.sum(unit_vectors, axis=1)

    # Repulsion
    F_repulsion = np.sum(-unit_vectors, axis=1)
    
    # Update velocity
    new_vel = vel + (w1 * F_attraction) + (w2 * F_repulsion)
...
ax.set_aspect('equal')
ax.set_title("Swarm Simulation (Unbounded Forces)")
...
ax_w2 = plt.axes([0.15, 0.1, 0.65, 0.03])
...
def step_frame(event):
    ...
    if not is_playing:
        positions, velocities = compute_update(positions, velocities, slider_w1.val, slider_w2.val)
        agent_trail.append(positions[TARGET_AGENT_IDX].copy())
